...
def filter_smorph_label(labels):
	...
	nlabels= len(labels)
	if nlabels==1 and (labels[0]=="COMPACT" or labels[0]=="BACKGROUND"):
		labels[0]= "NONE"
		
	return labels
	

def process_model_output_multiclass_multilabel(model_output, model_output, label, label2id, label_modifier_fcn=None):
