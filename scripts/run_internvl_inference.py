...
def get_args():
	...
	parser.add_argument('--imgsize', default=448, type=int, help='Image resize size in pixels')
	...
	# - Model option
	parser.add_argument('-model','--model', dest='model', required=False, type=str, default="OpenGVLab/InternVL2_5-1B-MPO", help='InternVL pretrained model from HF repo or local path') 
	...
	parser.add_argument('-device_map','--device_map', dest='device_map', required=False, type=str, default="auto", help='Device map where to run inference {auto,split}. Default is auto.') 
...
def main():
	...
	if args.benchmark=="smorph-rgz":
		...
		run_internvl_model_rgz_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
		...
		run_internvl_model_smorph_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
		...
		run_internvl_model_galaxy_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
		...
		run_internvl_model_artefact_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
		...
		run_internvl_model_anomaly_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
		...
		run_internvl_model_mirabest_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
		...
		run_internvl_model_gmnist_inference(
			datalist=datalist,
			model=model,
			tokenizer=tokenizer,
			resize_size=args.imgsize, 
