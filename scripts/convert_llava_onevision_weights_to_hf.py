...
def convert_llava_to_hf(model_path, model_id, pytorch_dump_folder_path, push_to_hub=False):

    # load original config
    #filepath = hf_hub_download(repo_id=model_id, filename="config.json", repo_type="model")
    filepath = os.path.join(model_path, "config.json")
    if not os.path.isfile(filepath):
        print("ERROR: Missing file %s!" % (filepath))
        return
    ...
    with torch.inference_mode():
        ...
        ##    ADDED BY SIMONE
        ############################
        # - Comment this as this works only for base models
        ###assert torch.allclose(outputs.logits[0, :3, :3], expected_slice, atol=1e-4) ## ORIGINAL CODE
        #assert torch.allclose(outputs.logits[0, :3, :3], expected_slice.half(), atol=1e-4) ## MODIFIED TO FIX TYPE MISMATCH
        #print("Logits are ok!")
        ############################ 
    ...
    print("--> Base model generated text: ", repr(expected_text))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to model name."
    )
    ...
    convert_llava_to_hf(args.model_path, args.model_id, args.pytorch_dump_folder_path, args.push_to_hub)
